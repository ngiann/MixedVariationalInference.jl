...
##########################################################
function plotellipse(q::AbstractMvNormal, clr="b", label=""; alpha=1.0, linewidth = 1)
##########################################################
    plotellipse(mean(q), cov(q), clr, label; alpha=alpha, linewidth = linewidth)
end

##########################################################
function plotellipse(μ, Σ, clr="b", label=""; alpha=1.0, linewidth = 1)
##########################################################
    ...
    PyPlot.plot(r_ellipse[:,1] .+ μ[1], r_ellipse[:,2] .+ μ[2], clr, linewidth=linewidth, label=label, alpha=alpha)
